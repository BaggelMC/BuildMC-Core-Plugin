...
public class ClaimRemove {
    ...
    public static int removeClaimCommand(@NotNull Player player, String type, String claimName) {
        ...
        switch (type.toLowerCase()) {
            ...
            default -> {
                 AudienceUtil.sendMessage(player, Component.translatable("messages.claims.remove.invalid-type"));
                CoreMain.soundManager.playSound(player, SoundUtil.mistake);
                return 0;
            }
        }
        ...
        if (claim == null) {
             AudienceUtil.sendMessage(player, Component.translatable("messages.claims.remove.failed"));
            CoreMain.soundManager.playSound(player, SoundUtil.mistake);
            return 0;
        }
        ...
        if (success) {
             AudienceUtil.sendMessage(player, Component.translatable("messages.claims.remove.success"));
